    ...
    for idx, cite in enumerate(citations):
        ...
        if source_content and len(source_content) > 0:
            ...
            if len(full_text) > 100:
                source_text += "..."
            source_text = html_mod.escape(source_text, quote=True)

        markers.append(
            f'<cite data-doc="{html_mod.escape(doc_title, quote=True)}" data-pages="{html_mod.escape(page_str, quote=True)}" data-text="{source_text}">[{idx + 1}]</cite> '
        )
...
async def get_history(agent, id, user_id: str):
    ...
    try:
        ...
        # Use aget_state to get current state
